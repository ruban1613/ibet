...
def test_otp_generation(token, student_id):
    ...
    # Test POST generate OTP for wallet operation
    data = {
        'operation_type': 'transfer_to_student',
        'amount': 200.00,
        'student_id': student_id,
        'description': 'Extra allowance for books'
    }
    response = requests.post(f"{BASE_URL}/parent/wallet/generate-otp/", headers=headers, json=data)
    print(f"   POST /parent/wallet/generate-otp/ - Status: {response.status_code}")
    if response.status_code == 201:
        otp_data = response.json()
        print("   ✅ OTP generated successfully")
        print("   🔒 OTP securely generated (not exposed in response)")
...
def test_error_scenarios(token):
    ...
    response = requests.post(f"{BASE_URL}/parent/wallet/generate-otp/", headers=headers, json=data)
    print(f"   POST /parent/wallet/generate-otp/ (invalid student) - Status: {response.status_code}")
